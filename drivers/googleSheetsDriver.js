...
// time.
const TOKEN_PATH = "./credentials/googleSheets/token.json";
let cachedAuth = null;

// Load client secrets
/*
const populateLookUpTables = () => {
  fs.readFile(
    "../credentials/googleSheets/googleSheetsCredentials.json",
    (err, content) => {
      if (err) return console.log("Error loading client secret file:", err);
      // Authorize a client with credentials, then call the Google Sheets API.
      authorize(JSON.parse(content), Tutor.populateCache);
      authorize(JSON.parse(content), Student.populateCache);
    }
  );
};
*/
